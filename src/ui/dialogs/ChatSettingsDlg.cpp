// ChatSettingsDlg.cpp: Implements the chat settings configuration dialog for NppOpenAI.
...
void ChatSettingsDlg::doDialog(bool)
{ // bool isRTL -- for modeless dialogs only!
...
INT_PTR CALLBACK ChatSettingsDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        if (::GetDlgItem(_hSelf, ID_PLUGINNPPOPENAI_CHATSETTINGS_LIMIT_STATIC) == (HWND)lParam || ::GetDlgItem(_hSelf, ID_PLUGINNPPOPENAI_CHATSETTINGS_INFO) == (HWND)lParam)
        {
            HDC hdc = (HDC)wParam;
